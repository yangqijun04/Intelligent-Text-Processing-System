...
EXTRACT_PROMPT = """你是战场通信情报分析专家。以下是标记为「{labels}」的通信记录。请提取所有关键要素，不得遗漏任何信息。

通信记录：
{filtered_lines}

要求：
1. 按类别分节输出Markdown，格式：## {labels}类关键要素
2. 逐条列出关键信息，每条标注来源行号：[行N]
3. 覆盖要素：时间、地点、坐标、兵力、装备、行动命令、伤亡、弹药消耗、补给请求、敌情动态、态势变化等
4. 只提取原文中明确存在的信息，不推断、不补充
5. 保留数字、代号、单位等精确值"""

VERIFY_PROMPT = """你是战场通信情报审核专家。请对照全文，检查以下提取结果是否有遗漏。
...
请逐条审查全文，输出：
1. ### 遗漏项清单
   列出所有未出现在提取结果中的关键信息（时间/地点/兵力/装备/行动/伤亡/补给/敌情），标注来源行号
2. ### 补充内容
   将遗漏项以上述格式补充完整
3. 如无遗漏，输出"### ✅ 检验通过，无遗漏"
不要重复已提取的信息。"""
...
class BatchClassifier:
    ...
    def generate_preprocessed_txt(results: list[dict]) -> str:
        ...
        lines = [r.get("preprocessed") or r.get("original", "") for r in valid]
        return "\n".join(lines)
